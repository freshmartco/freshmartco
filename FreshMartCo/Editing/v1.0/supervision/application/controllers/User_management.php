...
class User_management extends CI_Controller 
{
	...
	function edit_user()
	{
		if($this->session->userdata(USER_TYPE.'_login'))
		{
			...
			$data['user_details']=$this->user->get_user_master_details($id);
			$data['ip_config']=$this->user->get_user_ip_addresses($id);
	...
	{	
		if($this->session->userdata(USER_TYPE.'_login'))
		{
			$data=$this->user->update_user_details();			
			redirect('user_management');
		}
		else
		{
